...
import old.matchClass as matchClass
...
async def button(ctx):
     await ctx.send(content = "Message Here", 
     components = [Button(style = ButtonStyle.URL, label = "No", url="https://google.com"), 
     Button(style = ButtonStyle.blue, label = "Yes", custom_id = "xbutton")])
    ...
    if len(username.split('#')) != 2:
        ...
        if data == "invalid game index":
            ...
            if type == 'round-by-round':
                ...
                embed = discord.Embed(title = f"Round {round.number} Overview", description = f'Score: {round.getScore()}', color=0x00f900)
...
async def on_button_click(interaction):
    ...
    if interaction.component.label.startswith("Next Round"):
        ...
        if round == None:
            await interaction.respond(content = "That was the last round", components = [lastR])
        elif event == None:
            await interaction.respond(content = "That was the last event", components = [[lastE], [lastR,nextR]])
        else:
            embed = discord.Embed(title = f"Round {round.number} Overview", description = f'Score: {round.getScore()}', color=0x00f900)
            eventStats = f'Killer: {event.killer}\nDeather: {event.deather}\nWeapon: {event.getWeapon()}'
            ...
            embed.add_field(name = "Damage events:", value = event.getStats(), inline = True)
            embed.add_field(name = "Round Stats:",value = roundStats, inline = False)
            await interaction.respond(embed = embed, components = [[lastE,nextE],[lastR,nextR]])
    ...
    if interaction.component.label.startswith("Last Round"):
        ...
        if round == None:
            await interaction.respond(content = "That was the first round", components = [nextR])
        elif event == None:
            await interaction.respond(content = "That was the first event", components = [[nextE], [lastR,nextR]])
        else:
            embed = discord.Embed(title = f"Round {round.number} Overview", description = f'Score: {round.getScore()}', color=0x00f900)
            eventStats = f'Killer: {event.killer}\nDeather: {event.deather}\nWeapon: {event.getWeapon()}'
